...
#make a loop that iterates through the array
#make a loop that has if statement that cehcks whether the category is "Garbage and Refuse"
#if it is the right one add the value to new variable

sum_of_garbage_refuse = 0
sum_of_unsanitary = 0
sum_vegetation = 0
sum_of_animalsandpests = 0

violations.each do |h|
    if h[:violation_category] == "Garbage and Refuse"
        sum_of_garbage_refuse += h[:money_owed]
    end
end

violations.each do |h|
    if h[:violation_category] == "Unsanitary Conditions"
        sum_of_unsanitary += h[:money_owed]
    end
end

violations.each do |h|
    if h[:violation_category] == "Vegetation"
        sum_vegetation += h[:money_owed]
    end
end

violations.each do |h|
    if h[:violation_category] == "Animals and Pests"
        sum_of_animalsandpests += h[:money_owed]
    end
end

puts sum_of_garbage_refuse 
puts sum_of_unsanitary 
puts sum_vegetation 
puts sum_of_animalsandpests 


number_of_violations_garbage = 0
number_of_violations_unsanitary = 0
number_of_violations_vegetation = 0
number_of_violations_animalsandpests = 0


violations.each do |h|
    if h[:violation_category] == "Garbage and Refuse"
        number_of_violations_garbage += 1
    elsif h[:violation_category] == "Unsanitary Conditions"
        number_of_violations_unsanitary += 1
    elsif h[:violation_category] == "Vegetation"
        number_of_violations_vegetation += 1
    elsif h[:violation_category] == "Animals and Pests"
        number_of_violations_animalsandpests += 1
    end
end


puts number_of_violations_garbage
puts number_of_violations_unsanitary
puts number_of_violations_vegetation
puts number_of_violations_animalsandpests